...
{
   int	i;
   struct ban_list_element *banned_node, *prev_node;
   char	*nextchar;
   bool found = FALSE;
   long ban_time;
   ...
   for (nextchar = buf; *nextchar; nextchar++)
      *nextchar = tolower(*nextchar);

   i = 0;
   for (banned_node = ban_list; banned_node; banned_node = banned_node->next)
     if ((banned_node->type == BAN_SELF) && strstr(buf, banned_node->site)){
       found = TRUE;
       if ((ban_time = time(0) - banned_node->date) >= SECS_PER_REAL_DAY){
	 if (banned_node == ban_list)
	   ban_list = ban_list->next;
	 else {
	   for (prev_node = ban_list; prev_node->next != banned_node; 
		prev_node = prev_node->next)
	     ;
	   
	   prev_node->next = banned_node->next;
	 }
	 SEND_TO_Q("Removing ban.\r\n",d);
	 free(banned_node);
	 write_ban_list();
	 i = 0;
	 return(0);
   ...
   if (!found)
     ...
     return (24 - ban_time/SECS_PER_REAL_HOUR);
}
