...
struct board_info_type board_info[NUM_OF_BOARDS] = {
   { 2099, 0, 0, LEVEL_GOD, "boards/Board.mort" , 0 },
   { 2098, LEVEL_BUILDER, LEVEL_BUILDER, LEVEL_GRGOD, "boards/Board.immort" , 0 },
   { 2097, 0, 0, LEVEL_ASS_IMPL, "boards/Board.bug" , 0 },
   { 2096, LEVEL_BUILDER, LEVEL_BUILDER, LEVEL_GRGOD, "boards/Board.builder" , 0 },
   { 1227, LEVEL_BUILDER, LEVEL_BUILDER, LEVEL_IMPL, "boards/Board.builder2", 0},
   { 1241, LEVEL_BUILDER, LEVEL_BUILDER, LEVEL_IMPL, "boards/Staff_meeting",0},
   { 1210, LEVEL_BUILDER, LEVEL_IMPL, LEVEL_IMPL, "boards/Board.update", 0}
};
...
SPECIAL(gen_board)
{
   ...
   if ((board_type = find_board(ch)) == -1) {
      logg("SYSERR:  degenerate board!  (what the hell..)");
      return 0;
   }
...
void	Board_write_message(int board_type, struct char_data *ch, char *arg)
{
   ...
   if ((NEW_MSG_INDEX(board_type).slot_num = find_slot()) == -1) {
      ...
      logg("SYSERR: Board: failed to find empty slot on write.");
      return;
   }
   ...
   *(tmstr + strlen(tmstr) - 1) = '\0';
   ...
   act("$n begins to write a message on the board.",TRUE,ch,0,0,TO_ROOM);
   ...
   if (!num_of_msgs[board_type])
   ...
   else
    {
	 ...
	 if (MSG_HEADING(board_type, i))
	    ...
	    logg("SYSERR: The board is fubar'd.");
...
int	Board_display_msg(int board_type, struct char_data *ch, char *arg)
{
   ...
   if (MSG_SLOTNUM(board_type, ind) < 0 || 
      ...
      logg("SYSERR: Board is screwed up.");
      return 1;
   }
...
int	Board_remove_msg(int board_type, struct char_data *ch, char *arg)
{
   ...
   if (slot_num < 0 || slot_num >= INDEX_SIZE) {
      logg("SYSERR: The board is seriously screwed up.");
...
void	Board_load_board(int board_type)
{
   ...
   if (num_of_msgs[board_type] < 1 || num_of_msgs[board_type] > MAX_BOARD_MESSAGES) {
      logg("SYSERR: Board file corrupt.  Resetting.");
   ...
   for (i = 0; i < num_of_msgs[board_type]; i++) {
	 ...
	 logg("SYSERR: Error - malloc failed for board header");
	 exit(1);
      }
	 ...
	 if ((MSG_SLOTNUM(board_type, i) = find_slot()) == -1) {
	    logg("SYSERR: Out of slots booting board!  Resetting..");
	    Board_reset_board(board_type);
	    return;
	 }
	 if (!(tmp2 = (char *)malloc(sizeof(char) *len2))) {
	    logg("SYSERR: malloc failed for board text");
	    exit(1);
	 }
