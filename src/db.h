...
#define BUILDINGH_FILE  "text/building_h" /* building hints */

#define OBJ_LIMITS_FILE "misc/obj_limits" /* the objects limits data */
#define PLAYER_FILE2	"players-new"	/* the player database		*/
...
int	create_ol_entry(int obj_num);
...
/*structure for the object limits data */
struct obj_limit_type
{
    long obj_num;
    int no_stored;
};

struct obj_limit_list_type
{
    long obj_num;
    int no_stored;
    int no_loaded;
...
struct reset_com {
  ...
  int	arg3;       /*                                      */
  int   percent;    /* percent chance of reset */
  int   num;        /*  number of times this reset has triggered */
  bool  twinkie; /* set for mobiles if they have too much good equipment */
   /* 
	*  Commands:              *
	*  'M': Read a mobile     *
	*  'O': Read an object    *
...
/* struct for creating a sorted list of objects */

struct zone_obj_list {
  int vnum;
  struct obj_data *obj;
  struct zone_obj_list *next;
};
